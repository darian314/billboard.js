  ...
  export: function _export(mimeType, scale, callback) {
    ...
    if (isFunction(callback)) {
      ...
      img.crossOrigin = "Anonymous", img.onload = function () {
        ...
        canvas.width = size.width * scale, canvas.height = size.height * scale, ctx.fillStyle = 'white', ctx.fillRect(0, 0, canvas.width, canvas.height), ctx.drawImage(img, 0, 0, size.width * scale, size.height * scale), callback(canvas.toDataURL(mimeType));
      }, img.src = svgDataUrl;
    }
